...
import 'package:flutter/material.dart'; // For showing messages potentially
import 'package:flutter_contacts/flutter_contacts.dart' as fc; // Use prefix 'fc'
  ...
  Future<ContactImportResult> fetchAndFilterDeviceContacts() async {
    ...
    if (status.isPermanentlyDenied) {
      ...
      return ContactImportResult(
          ...
          errorMessage: 'Contacts permission permanently denied. Please enable in settings.');
    }
    ...
    List<fc.Contact> filteredContacts = deviceContacts.where((contact) {
        ...
        final bool hasCompany = contact.organizations.isNotEmpty &&
                               (contact.organizations.first.company?.isNotEmpty ?? false);
        // Check if first and last names are both empty or null
        final bool nameIsEmpty = (contact.name.first.isEmpty) && (contact.name.last.isEmpty);
        ...
        if (shouldRemove) {
          logger.d("Filtering out business contact: ${contact.displayName} (Company: ${contact.organizations.first.company})");
        }
    ...
    if (originalCount != filteredCount) {
         logger.i("Filtered out ${originalCount - filteredCount} business-only contacts.");
    } else {
    ...
    // --- 4. Return Success ---
    logger.i("Contact fetching and filtering complete. Returning ${filteredContacts.length} contacts.");
    return ContactImportResult(
